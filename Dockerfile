...
FROM --platform=linux/amd64 golang:1.17.5-alpine AS build
...
LABEL org.opencontainers.image.revision=$VCS_REF \
	org.opencontainers.image.source="https://github.com/masaruhoshi/${PKG_NAME}"

COPY --from=build /etc/ssl/certs/ca-certificates.crt /etc/ssl/certs/ca-certificates.crt
COPY --from=build /bin/${PKG_NAME}_${TARGETOS}-${TARGETARCH}${TARGETVARIANT} /${PKG_NAME}

ENTRYPOINT [ "/mongodb_atlas_prometheus_exporter" ]
